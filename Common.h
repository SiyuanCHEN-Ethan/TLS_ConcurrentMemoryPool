...
//主要包括了一个FreeList类、Span结构体、SpanList类 以及 计算大小位置的SizeClass类
...
{
private:
	void* _list = nullptr; // 给上缺省值 CSY：_list指向第一个对象
	size_t _size = 0;  // 记录有多少个对象
	size_t _maxsize = 1;

public:

	void Push(void* obj) //CSY：存入一个对象
	{
		NEXT_OBJ(obj) = _list; //CSY：类似在链表头部插入一个节点，先将原本的第一个对象_list存给obj的下一个对象
		_list = obj; //然后将obj给到头节点
		++_size;
	}

	void PushRange(void* start, void* end, size_t n)//CSY：存入N个对象
	{
		NEXT_OBJ(end) = _list;
		_list = start;
		_size += n;
...
public:
	//获取Freelist的位置
	inline static size_t _Index(size_t size, size_t align)  //CSY：静态成员函数只能访问静态成员，静态成员即所有的对象只能有这一个副本
	{
		size_t alignnum = 1 << align;  //这是参考了库里实现的方法
...
public:
	...
	inline static size_t Index(size_t size) //CSY：返回的是这个大小的内存应该位于thread cache的 freelist的第index个
	{
		assert(size <= MAX_BYTES); //CSY注：断言，在运行时确保传传入参数不越界
	...
	// 对齐大小计算，向上取整
	static inline size_t Roundup(size_t bytes) //CSY注：按向上取整的大小取内存。 比如这里输入bytes为63，返回的可能就是64
	{
	...
	// 根据size计算中心缓存要从页缓存获取多大的span对象（span对象的大小为多少页）
	static size_t NumMovePage(size_t size)
	{
		size_t num = NumMoveSize(size);
		size_t npage = num * size;
		npage >>= PAGE_SHIFT; //对应1页大小为4k
		if (npage == 0)
			npage = 1;
		return npage;
	}
...
struct Span
{
	...
	size_t _objsize = 0;//span上挂载的对象的大小
...
public:
	...
	Span* End()//最后一个的下一个指针 CSY：回到了head
	{
		return _head;
	}
	...
	bool Empty()
	{
		return _head->_next == _head; //CSY : 对环形链表的判断
	}

	//在pos位置的前面插入一个newspan
	void Insert(Span* cur, Span* newspan) // CSY ： 在节点cur前面插入一个newspan
	{
		Span* prev = cur->_prev;
	...
	{
		Insert(Begin(), newspan); //在head的前面插入
	}
	...
	void Lock()
	{
		_mutex.lock();//对每一个spanList类，有一把锁
	}
