...
void* ThreadCache::FetchFromCentralCache(size_t index, size_t size) //CSY： 调用的时候输入的size一般需要先roundup过
{
	//打桩
	//return malloc(size);
	...
	size_t numtomove = min(SizeClass::NumMoveSize(size), maxsize); //取回来要分配的数量取决于两者中较小的那个；maxsize为freelist的容量上限；NumMoveSize为根据size算出来的数量
...
void ThreadCache::Dealloctor(void* ptr, size_t size)
{
	size_t index = SizeClass::Index(size);
	Freelist* freelist = &_freelist[index];
	freelist->Push(ptr); // CSY：将内存回收到线程cache的 freelist中
